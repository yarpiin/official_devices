...
if [ -n "$PULL_REQUEST_NUMBER" ]; then
    ...
    sendAdmins "**I am building master branch job.** %0A**Commit Point:** [${COMMIT_SMALL_HASH}](https://github.com/Evolution-X-Devices/official_devices/commit/${COMMIT_HASH})"
fi
...
if [ -n "$PULL_REQUEST_NUMBER" ]; then
    ...
    if [ "$RESULT" -eq 1 ]; then
        ...
        sendAdmins "\`PR $PULL_REQUEST_NUMBER can be merged.\` %0A%0A${ADMINS} %0A%0A[PR Link](https://github.com/Evolution-X-Devices/official_devices/pull/$PULL_REQUEST_NUMBER)"
